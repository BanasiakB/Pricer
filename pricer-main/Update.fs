...
let tradeChangeUpdate (model : Model) = function
    | NewName (id,name) ->
        changeTrade model.trades id 
                (Trades.tryMap ( function
                                ...
                                | Option o -> Some <| Option { o with OptionName = name}))
        ...
        changeTrade model.trades id 
                (Trades.tryMap ( function
                                ...
                                | Option o -> 
                                    ...
                                    |> Utils.ofBool
                                    |> Option.map (fun expiry ->
                                            Option { o with Maturity = expiry})))

    | NewCurrency (id,ccy) ->
        changeTrade model.trades id 
                (Trades.tryMap ( function
                                | Payment p -> Some <| Payment { p with Currency = ccy}
                                | Option o -> Some <| Option {o with Currency = ccy}))

    | NewAssetPrice (id, price) ->
        changeTrade model.trades id 
                (Trades.tryMap (function
                    | Option o ->
                        Int64.TryParse(price)
                        |> Utils.ofBool
                        |> Option.map (fun price ->
                                            Option {o with AssetPrice = price})
                    | Payment _ -> None))

    | NewStrike (id: TradeID, strike) -> 
        changeTrade model.trades id 
            (Trades.tryMap (function 
                    | Option o ->
                        Int64.TryParse(strike)
                        |> Utils.ofBool
                        |> Option.map (fun strike ->
                                            Option {o with Strike = strike})
                    | Payment _ -> None))
    
    | NewInterestRate (id, rate) ->
        changeTrade model.trades id 
                (Trades.tryMap (function
                        | Option o -> 
                            rate.Replace(".", ",") |> Double.TryParse |> Utils.ofBool 
                            |> Option.map (fun value -> Option {o with InterestRate = value})
                        | Payment _ -> None))
    ...
    | NewVolatility (id, volatility) ->
        changeTrade model.trades id 
            (Trades.tryMap (function
                        | Option o -> 
                            volatility.Replace(".", ",") |> Double.TryParse |> Utils.ofBool 
                            |> Option.map (fun value -> Option {o with Volatility = value})
                        | Payment _ -> None))

    | NewOptionType (id, optionType) ->
        changeTrade model.trades id 
            (Trades.tryMap (function
                            | Option o -> Some <| Option { o with OptionType = optionType}
                            | Payment _ -> None))
...
let update (http: HttpClient) message model =
    ...
    | AddOption ->
        let newOption = Trades.wrap (Option <| OptionRecord.Random())
    ...
    | LoadData ->
        let getConfig() = http.GetFromJsonAsync<JsonConfig>("/configuration1.json")
